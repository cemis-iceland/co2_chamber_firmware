...
struct Config {
  ...
  int air_meas_interval =
      1; // The interval at which the BME280 takes measurements in seconds
  int soil_meas_interval = 60; // The interval at which the soil temp and soil
...
void enterWarmup() {
  ...
  xTaskCreatePinnedToCore(measure_co2_task, "measure_co2", 16384, NULL, 1,
                          &measure_co2, 1);
  xTaskCreatePinnedToCore(measure_air_task, "measure_air", 16384, NULL, 1,
                          &measure_air, 1);
  xTaskCreatePinnedToCore(measure_soil_task, "measure_soil", 16384, NULL, 1,
                          &measure_soil, 1);
}
